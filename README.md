...
*   **士气溃散机制**：仿照《战场兄弟》实现的连锁恐惧系统——友军阵亡、重伤或目睹同袍逃跑都会触发士气检定，可能引发全军溃败。
...
| 士气状态 | 命中修正 | 伤害修正 | 防御修正 | 特殊效果 |
| :--- | :---: | :---: | :---: | :--- |
| 😤 自信 | +10% | +10% | +5% | 战斗加成 |
| 😐 稳定 | - | - | - | 正常状态 |
| 😰 动摇 | -5% | - | -5% | 开始犹豫 |
| 😱 崩溃 | -15% | -5% | -10% | 25%概率跳过行动 |
| 🏃 逃跑 | -30% | -20% | -20% | 失去控制，自动远离敌人 |
...
## 开发路线图 (Roadmap)
- [x] **士气溃散系统**：实现在战斗中因队友阵亡导致的士气连锁崩溃，包含完整的士气检定、连锁恐惧、AI士气行为。
- [ ] **永久伤残系统**：角色虽未战死，但可能失去一只眼或一条腿，转化为永久属性扣减。
- [ ] **AI 增强**：实现敌方的合围、退避等高级 AI 策略。
- [ ] **动态贸易**：城邑的物资价格受战争、饥荒等状态影响波动。
- [ ] **大国博弈事件**：随天数推进触发历史大事件（如：长平之战），影响全图势力分布。
